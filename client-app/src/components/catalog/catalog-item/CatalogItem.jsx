        ...
        <article key={post._id} className="flex max-w-xl flex-col items-start justify-between border-4 border-gray-300 rounded-xl p-6 shadow-lg bg-white">
            ...
            <div className="flex gap-12 mt-4">
                <Link to={`/posts/${post._id}/details`} className="px-4 py-1.5 bg-blue-500 text-white rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400">See post</Link>
                {
                   ...
                   */
                        ?
                        <Link onClick={() => toggleLike(post._id)} className="px-4 py-1.5 bg-blue-500 text-white rounded focus:outline-none focus:ring-2 focus:ring-green-400" >
                ...
                }
                {
                    userId && userId !== post._ownerId && isLiked
                                        ?
                    <Link onClick={() => toggleDislike(post._id)} className="px-4 py-1.5 bg-red-500 text-white rounded focus:outline-none focus:ring-2 focus:ring-green-400">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
